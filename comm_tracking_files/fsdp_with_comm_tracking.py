# this is an instrumented version of fsdp to track all_gathers and reduce_scatters
# set debug_comms to True on init, will print comm type, obj id, training state, dtype and size
...
class FullyShardedDataParallel(nn.Module):
    ...
    def _need_prefetch_full_params(self, state: TrainingState_) -> bool:
        allowed_states = (
            ...
            TrainingState_.BACKWARD_PRE,
            TrainingState_.BACKWARD_POST,
        )
        assert (
            state in allowed_states
        ), f"state needs to be in the set of {allowed_states}"
    ...
    def _sharded_post_state_dict_hook(
        ...
        for (
            ...
            _,
            module_name,
        ) in self._fsdp_wrapped_module.handle.flat_param._param_infos:
            module_name = module_name.replace(f"{FPW_MODULE}.", "")
            module_name = module_name.replace(f"{FPW_MODULE}", "")
            if module_name:
                module_name = f"{module_name}."
    ...
    def state_dict(self, *args, **kwargs):
        ...
        if self._state_dict_type == StateDictType.FULL_STATE_DICT:
            ...
            if (
                ...
                and not self._fsdp_wrapped_module.flat_param._is_sharded
            ):
                raise RuntimeError(
    ...
    def _local_pre_load_state_dict_hook(
        ...
        if fqn not in state_dict:
            assert (
                getattr(self._fsdp_wrapped_module, FLAT_PARAM, None) is None
    ...
    def forward(self, *args: Any, **kwargs: Any) -> Any:
        with torch.autograd.profiler.record_function(
            ...
            if self._need_prefetch_full_params(self.training_state):
                ...
                self._streams["all_gather"].wait_stream(torch.cuda.current_stream())
                self._fsdp_graph_order[
                    self._my_fsdp_idx_in_graph + 1
                ]._rebuild_full_params()
                self._fsdp_graph_order[
                    self._my_fsdp_idx_in_graph + 1
                ]._forward_full_params_prefetched = True
    ...
    def summon_full_params(
        ...
        fsdp_modules = FullyShardedDataParallel.fsdp_modules(module, root_only=True)
        ...
        def _pre_backward_hook(*unused: Any) -> None:
            ...
            with torch.autograd.profiler.record_function(
                ...
                if self._need_prefetch_full_params(self.training_state):
                    ...
                    self._fsdp_graph_order[
                        self._my_fsdp_idx_in_graph - 1
                    ]._pre_backward_hook_full_params_prefetched = True
        ...
        with torch.autograd.profiler.record_function(
            ...
            with torch.cuda.stream(self._streams["post_backward"]):
                ...
                if (
                    ...
                    and self.communication_hook is None
                ):
                ...
                if param._is_sharded:  # type: ignore[attr-defined]
                    ...
                    if self.debug_comms:
                        print(
                            f"reduce_scatter: id = {id(param)}, state = {self.training_state}, dtype = {input_flattened.dtype}, size = {input_flattened.shape[0]}"
                        )
                    ...
                    if self.sharding_strategy == ShardingStrategy.NO_SHARD:
                        ...
                        self.communication_hook(
                            self.communication_hook_state, param.grad
                        )
            ...
            if isinstance(m, FullyShardedDataParallel):
                if any(p.requires_grad for p in m.parameters()):
                    ...
                    if managed_param_requires_grad:
                        ...
                        post_backward_hook_called = any(
                            p._post_backward_called for p in m.params
                        )
                        if post_backward_hook_called:
                            ...
                            # not get gradient
                            m._assert_state(
                                [TrainingState_.BACKWARD_PRE, TrainingState_.IDLE]
                            )
                    else:
        ...
        with torch.cuda.stream(self._streams["all_gather"]):
            for p in self.params:
                ...
                # If full param has been rebuilt or has not been freed, no need to call all gather
                    ...
                    if self.debug_comms:
                        print(
                            f"all_gather: id = {id(p)}, state = {self.training_state}, p_data dtype = {p_data.dtype}, size = {p_data.shape[0]}"
                        )
    ...
    def full_optim_state_dict(
        ...
        num_missing = torch.tensor(
            [len(missing_keys)], dtype=torch.int32, device=device
        )
    ...
    def scatter_full_optim_state_dict(
        ...
        if rank == 0:
            ...
            flat_osd = _flatten_full_optim_state_dict(
                full_optim_state_dict, model, False
            )
    ...
    def register_comm_hook(self, state: object, hook: callable):
        ...
        if self.sharding_strategy != ShardingStrategy.NO_SHARD:
            ...
            for submodule in self.fsdp_modules(self):
                ...
                assert (
                    submodule.communication_hook == self._get_default_comm_hook()
        ...
        if not isinstance(module, FullyShardedDataParallel):
            for param_name, param in module.named_parameters(recurse=False):
                ...
                if not is_shared_param:
                    ...
                    param_to_unflat_param_names[param].extend(
                        fully_prefixed_param_names
                    )
