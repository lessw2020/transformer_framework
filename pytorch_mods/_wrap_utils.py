...
def _auto_wrap(
    ...
    mp_modules_to_override = [_BatchNorm, LayerNorm]
    if mixed_precision is not None and any(
        _contains_module(root_module, module_to_override) for module_to_override in mp_modules_to_override
    ):
        for mp_module_to_override in mp_modules_to_override:
            _override_module_mixed_precision(root_module, mp_module_to_override)

        auto_wrap_policy = functools.partial(
            _or_policy, policies=[auto_wrap_policy, _wrap_batchnorm_individually, _wrap_layernorm_individually])
        warnings.warn(
            "Both mixed precision and an `auto_wrap_policy` were specified "
            "for FSDP, where the wrapped module has batch norm / layer norm submodules. "
